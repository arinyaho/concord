...
A confirmed action runs as the existing capability job (the same container launcher, credential isolation, and single-slot-per-repo interlock as a task submitted through the capability channel above) -- it does not open a second execution path. The job's outcome (`[job result: alias=..., branch=..., outcome=..., summary=...]`) is fed back into the SAME conversation thread as a synthesized turn, so the roles react to what actually happened rather than what was proposed.
