...
export function makeRunCli({ repoRoot, stateDir, cliPath, timeoutMs = 600000 }) {
  return function runCli(verb, args = []) {
    return new Promise((resolve) => {
      ...
      const timer = setTimeout(() => {
        child.kill("SIGKILL");
        resolve({ harnessFailure: true, message: `review-cli ${verb} timed out after ${timeoutMs}ms` });
      }, timeoutMs);
      timer.unref?.();
      child.on("close", (code) => {
        clearTimeout(timer);
      ...
      child.on("error", (e) => {
        clearTimeout(timer);
        resolve({ harnessFailure: true, message: e.message });
      });
    });
  };
}
