...
async function main() {
  ...
  const runJob = (job) => {
    ...
    return runLaunchJob({ argv, env: childEnv, onChild: (c) => { job.child = c; }, onProgress: (event) => job.onProgress?.(event) }, { spawn: nodeSpawn });
  };
  ...
  const { rawPost, postSystem } = makeInlinePosters({ fetchChannel: (id) => client.channels.fetch(id) });
