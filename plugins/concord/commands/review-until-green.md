...
4. `node ".../review-cli.js" plan-fixes <ref>` -> `{ fixes: [{id, file, span, summary}, ...] }`.
5. For EACH fix in order (sequentially, one at a time -- never in parallel; two fixes may touch the same file): spawn a fix subagent that applies the minimal correct fix by editing the working tree, then writes ONLY `{ "status":"ok", "edited": true }` to `<stateDir>/round-<n>-fix-<id>.json` (`edited:false` if it decided no change was warranted). Wait for it, then immediately run `node ".../review-cli.js" commit-fix <ref> <id>` -> `{committed, sha?, reason?}` BEFORE spawning the next fix subagent -- this commits only that one fix's edit while it is the sole uncommitted change, so two fixes touching the same file land as two separate, correctly attributed commits.
6. `node ".../review-cli.js" record <ref>` -> `{ decision, handoff }`.
