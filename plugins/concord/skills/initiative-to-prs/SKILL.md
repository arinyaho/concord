...
When a downstream ticket in the same repository depends on an unmerged prerequisite PR, use an explicit stacked delivery: branch from the prerequisite PR head, target the downstream PR at the prerequisite branch, and record the stack in both handoffs and PR bodies. A verified stacked PR URL with its prerequisite branch as base is a completion disposition; report it as pending retarget after the prerequisite merges. If repository or provider rules do not permit that stack, keep the downstream ticket blocked until the prerequisite is integrated.
