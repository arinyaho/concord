...
When a downstream ticket in the same repository depends on an unmerged prerequisite PR, use an explicit stacked delivery: branch from the prerequisite PR head, target the downstream PR at the prerequisite branch, and record the stack in both handoffs and PR bodies. The final-mutations role owns a dependency-blocked retarget handoff: after the prerequisite merges, resume that handoff to retarget the downstream PR to the repository's normal base and read it back; do not exit the initiative successfully before that handoff passes. If repository or provider rules do not permit that stack, keep the downstream ticket blocked until the prerequisite is integrated.
