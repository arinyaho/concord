...
The active agent may perform that pass when its resolved model is deep-capability. Otherwise use an authorized specialist with clean context or switch the stage to a deep model. For a material cryptography, security, or migration decision, also require a separate deep-capability reviewer, even when the responsible agent is deep-capability. Build its packet from source evidence and the proposed decision, without the responsible agent's reasoning; ask it to seek counterexamples and failure modes. Reconcile substantive disagreement before dependent work continues. If required deep capability or independent review is unavailable, leave the decision unresolved and do not create dependent tickets. A general model may handle ordinary ticket decomposition after the architecture contract is settled.
