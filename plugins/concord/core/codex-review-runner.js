...
function reviewerPrompt(role, { stateDir, round, targetType, dodPassed, dodDeferred, finding, retryPrompt, slug, priorIntentIds, plannedFindingIds = [] }) {
  ...
  if (role === 'fix') {
    const mirrorClaim = targetType === 'file' ? '' : `,"resolvedFindingIds":["<distinct planned mirror finding id>"]}. Omit resolvedFindingIds unless this commit also resolves that distinct planned finding: its exact span must be absent from a file in files. It may contain only IDs from this round's other planned fixes: ${JSON.stringify(plannedFindingIds.filter((id) => id !== finding.id))}`;
    return `Apply the minimal correct fix for ${finding.id} at ${finding.file}, ${finding.span}: ${finding.summary}. Edit only necessary files. Then write ONLY to ${artifact}: either {"status":"ok","edited":false} if no change was warranted, or {"status":"ok","edited":true,"files":["<every edited path>"]}${mirrorClaim}. The files array MUST truthfully list EVERY file edited, including required companion files, as repository-relative paths. It MUST NOT include this state artifact, any stateDir artifact, or any path outside the repository.${retry}`;
  }
