...
## RESOLVED: `lifecycle` and `transcript`

Codex's hook payload and rollout format were characterized and implemented in `adapters/codex/event.js` and `adapters/codex/transcript.js`. The installed plugin vendors both adapters and uses them for its session-state checkpoint and charter hooks.

One durability difference remains: Claude Code can locate an abandoned session transcript beside its project state directory and fold an unwritten tail on the next start. Codex rollouts live in a date-partitioned global session tree unrelated to Concord's cwd-derived state directory, so Codex persists each session through its own Stop hooks but cannot perform that abandoned-tail catch-up scan.
