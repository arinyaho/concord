...
- **Trust-table registration is a non-issue.** A real git repo that isn't yet in Codex's trust table self-registers on first `codex exec` and proceeds without an interactive prompt, so no extra flag is needed to avoid a hang here. The only case that flag family addresses is `--cd` targeting a directory that is not a git repository at all, which never happens in this driver since `review-until-green` always runs against a git repo (it reviews a `git diff`).
