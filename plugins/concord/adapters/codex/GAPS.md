...
- **Trust-check hang risk.** An unattended `codex exec` invocation against a repo that isn't already in Codex's trust table hangs on stdin waiting for an interactive trust prompt, rather than failing fast — fatal for a driver that spawns 15-20 of these per broad round with nothing watching stdin. `--skip-git-repo-check` (now included in every `codex exec` invocation in `spawn-include.md`) avoids the prompt, but it is worth noting as a sharp edge for anyone hand-rolling a `codex exec` call outside the spawn-include's template.
