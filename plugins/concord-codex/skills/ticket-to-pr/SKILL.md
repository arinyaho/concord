...
| 9 | One PR | Design, docs and code in the same PR; the repository's PR template followed; every document contradicted by the change corrected in it; a supplied Notion ticket contains the PR URL and is `In review`, or remains a later or terminal status, both verified |
...
When the input work item is a Notion ticket, the `ticket-to-pr` request authorizes its lifecycle updates. At pipeline entry, before stage 3, read the ticket and its database schema, and classify the current status before discovering the editable URL fields. If it already means `In review` or `Done`, preserve it and read the ticket back to verify the status without requiring an `In progress` status mapping; preserve an existing `In progress` and every later or terminal status likewise. Transition it only when the current status is explicitly an unambiguous pre-start status and exactly one status in the database means `In progress`, then read the ticket back to verify the status. If the current status cannot be classified, a transition is needed but the status match is zero or multiple, or the provider rejects the transition, stop and report the blocker.

After the PR URL exists, an eligible PR URL field is an editable URL field explicitly for the PR. When exactly one is eligible, write it only when that field is empty or already has the same URL; if it contains a different URL, preserve it and use the ticket-body fallback. If no eligible PR URL field exists, use the ticket-body fallback. If multiple eligible PR URL fields exist, do not choose one: use the ticket-body fallback and record the ambiguous mapping; the ticket-body fallback can complete after verification. Before any ticket-body fallback, inspect it for the same labelled `PR:` link and append only when absent. Read the ticket back to verify the exact PR URL before performing the status transition, and stop if it is absent. If the ticket is already `In review`, later, or terminal, preserve it without requiring an `In review` status mapping; otherwise, only from `In progress` and with exactly one status in the database that means `In review`, may it transition. Only then transition it and read the ticket back to verify the exact URL and status. If either update fails, the current status cannot be classified, or a transition is needed but the status match is zero or multiple, report the failure; do not claim the stage completed. Do not move the ticket to Done: merge remains outside this pipeline.
