...
When a downstream ticket in the same repository depends on an unmerged prerequisite PR, use an explicit stacked delivery: branch from the prerequisite PR head and target the downstream PR at the prerequisite branch. For every downstream diff review, override `ticket-to-pr`'s default invocation with `/review-until-green <downstream-branch> <prerequisite-branch>` so the review covers only the downstream PR diff. A verified stacked PR is a completion disposition when its handoff and PR body record the prerequisite branch, normal base, and named owner for the follow-up. After the prerequisite merges, that owner fetches the live downstream PR head and integrated base, checks out the local downstream branch, resets it to the exact fetched PR head, retargets the PR, and checks its three-dot diff. If prerequisite commits remain in that diff, as can happen after a squash or rebase merge, the owner restacks only the downstream changes onto the integrated base and updates the PR head. Before treating the follow-up as verified, the owner first re-arms the clean review ledger with `review-cli.js rerun <downstream-branch>` even if the head is unchanged, then reviews the resulting diff with `/review-until-green <downstream-branch> <integrated-base>` using the fetched base ref or SHA, reruns its required checks against that base, pushes the final reviewed commit, waits for required checks on that exact remote head, and reads back the final head SHA, base, diff, and check results. This follow-up is outside initiative completion because this pipeline does not merge PRs. If repository or provider rules do not permit that stack, keep the downstream ticket blocked until the prerequisite is integrated.
