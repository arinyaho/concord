...
- For every ticket's diff review, override `ticket-to-pr`'s default invocation with `/review-until-green <branch> <target-base>`, using the target base recorded in the execution handoff. For ordinary tickets, fetch the target remote and resolve the base to its remote-tracking ref or an immutable fetched SHA. For stacked tickets, use the fetched prerequisite head. Pass that base explicitly on every diff-review run so the gate covers only the ticket's PR diff. When an independent reviewer role is authorized, give that reviewer the contract and evidence packet separately; it must not inherit the implementer's reasoning context.
...
2. After the approved tickets and external design records have been written and read back, present their URLs, dependency order, acceptance criteria, Definition of Done, and preserved metadata. For repository-backed design records, present the assigned ticket and planned branch and PR disposition; read those records back with that ticket's implementation PR. Continue to implementation only when the user approves the read-back ticket set.
