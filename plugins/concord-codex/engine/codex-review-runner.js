...
function reviewerPrompt(role, { stateDir, round, targetType, dodPassed, dodDeferred, finding, retryPrompt, slug, priorIntentIds }) {
  ...
  if (role === 'correctness') {
    ...
    return `${doc ? 'Review the document' : 'Review the diff and surrounding code'} at ${path.join(stateDir, `round-${round}-diff.txt`)}. ${doc ? 'Find contradictions, unsupported claims, placeholders, over-claims, and omitted limitations. Every reviewed target MUST appear in "examined". Finding IDs MUST use docreview:<stable-slug>.' : `Find correctness bugs, reuse/efficiency problems, and verifier-gaming. ${dodNote} IDs must start correctness:. Every changed file in the diff MUST appear in "examined". Uncertainty is not a reason to withhold a finding: a separate verify pass rejects false positives, so a finding you are unsure about costs nothing to raise. Triviality is different -- a real but minor finding is not a false positive, so it routes to a fixer and an honest "no change warranted" parks the run.`} Write ONLY JSON to ${artifact}: {"status":"ok","examined":[],"findings":[]}.${retry}`;
  }
