...
Before checkpoint 1, have a deep-capability model examine compressed evidence, alternatives, counterexamples, reversibility, migration cost, and acceptance checks for material choices about cryptographic format or protocol, security, authorization, identity, data layout, deployment boundaries, public API behavior, or ticket splits that lock in those choices. This applies even without conflicting sources. The active agent may do the pass only when its handoff records the resolved model and catalog-backed evidence that it has deep capability; otherwise use an authorized specialist or switch the stage model. For a material cryptography, security, or migration decision, require a separate deep-capability reviewer even when the active agent is deep-capability. Give it source evidence and the proposed decision without the responsible agent's reasoning, and reconcile substantive disagreement before dependent work continues. If required deep capability or independent review is unavailable, leave the decision unresolved and do not create dependent tickets.
