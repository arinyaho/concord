...
**Reconciliation with the headless E2E (added after implementation):** the "completed without ever blocking on stdin/approval" observation above held for this spike's session, which was run interactively/attached to a TTY. The headless adapter path is different in a load-bearing way: the review/fix subagents are spawned as background subprocesses whose stdin is not an interactive terminal (and is not closed either), and against an equally-untrusted scratch repo, that unattended `codex exec` DID hang on Codex's trusted-directory prompt when `--skip-git-repo-check` was omitted — it sat waiting on a prompt no one could answer. So the flag is not optional for the adapter's actual spawn path; the finding above should be read as "no extra flag needed for an interactive/TTY run," not "no extra flag needed, period." See the E2E note for the hang and the fix.
