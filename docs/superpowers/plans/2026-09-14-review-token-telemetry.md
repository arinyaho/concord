...
2. Add a shared review CLI operation used immediately before every Codex subprocess or manual Claude `Agent` review/fix launch, including retries and failures, to persist a deterministic `(engine, provider, destination, attempt number)` slot without a pre-known tool ID or prompt change. Reconcile each provider only against its owned slots, require exactly one telemetry record per slot, and mark missing, duplicate, or orphan evidence partial; then make `invoke` return and record each normalized Codex result.
...
5. Before each manual `Agent` launch, persist its expected Claude-owned `(engine, provider, destination, attempt number)` through the shared review CLI operation, then fold matching tool and agent records against only those slots into the run ledger and terminal handoff. Deduplicate by invocation ID, require the pinned Claude Code/transcript schema, and mark missing or inconsistent joins partial without changing prompts, scheduling, or terminal decisions.
